			...
			if (ordineDb.isEmpty()) {
			    ...
			    for (User user : allUsers) {
			        ...
			        for (int i = 0; i < numOrders; i++) {
			            LocalDate orderDate = faker.date().birthday().toInstant().atZone(ZoneId.systemDefault()).toLocalDate(); // Generate a random order date
			            LocalTime orderTime = faker.date().birthday().toInstant().atZone(ZoneId.systemDefault()).toLocalTime(); // Generate a random order time
